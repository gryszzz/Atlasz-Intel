...
export const MARKET_DATA_SURFACES: MarketDataSurface[] = [
  ...
  // --- Simulated / seeded (dev-only) — NOT allowed in production ---
  {
    ...
    currentlyRenderedInProduction: true,
    marketRelevance: 'high',
    note: 'Hardcoded demo OHLC/price series. Must be gated behind the dev flag or removed from production charts.',
  },
  {
    ...
    currentlyRenderedInProduction: true,
    marketRelevance: 'high',
    note: 'Seeded prices/percent moves with no source/retrievedAt/hash. Gate behind dev flag or replace with the real price provider.',
  },
  {
    id: 'seed-top-signals',
    ...
    currentlyRenderedInProduction: true,
    marketRelevance: 'high',
    note: 'Seeded signal/conviction scores not backed by a source. Must not appear as real market signals in production.',
  },
  {
    ...
    currentlyRenderedInProduction: true,
    marketRelevance: 'medium',
    note: 'Demo radar events. Real events come from worldSnapshot.worldEvents (source-backed); seed should be dev-only fallback.',
  },
  {
    id: 'mock-market-tape',
