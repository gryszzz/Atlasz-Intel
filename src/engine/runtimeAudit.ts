...
export const CONNECTOR_AUDIT_DEFINITIONS: ConnectorAuditDefinition[] = [
  ...
  connector({
    ...
    trust: 'official-api',
    notes: 'Official macro observations. Public FRED graph CSV by default; optional API key upgrades REST metadata. No synthetic macro charts or commentary.',
  }),
  connector({
    id: 'treasury-fiscal',
  ...
  connector({
    id: 'eia-bulk-public',
    label: 'EIA Public Bulk',
    domain: 'energy and commodities',
    sourceIds: ['eia_bulk_public'],
    access: 'public',
    cadence: 'periodic',
    sourceIdentity: 'U.S. Energy Information Administration public bulk files',
    officialUrl: 'https://www.eia.gov/opendata/bulkfiles.php',
    requiredEnv: [],
    persistenceTables: ['eia_energy_records', 'world_intel_events', 'source_audit_log'],
    sourceTrailUi: 'Energy source trail cards',
    resolverSupport: 'yes',
    exposureSupport: 'curated-reference',
    trust: 'official-api',
    notes: 'No-key EIA public bulk reference for a bounded allowlisted series subset. Does not unlock the authenticated EIA API, facilities, grid, refineries, or LNG layers.',
  }),
  connector({
    id: 'noaa-alerts',
  ...
  connector({
    ...
    exposureSupport: 'future',
    trust: 'official-api',
    notes: 'Official bill/action metadata only. Uses DEMO_KEY mode by default; personal key is quota-only. No fake bills, political interpretation, person enrichment, or inferred company exposure.',
  }),
  connector({
    id: 'usgs-earthquakes',
  ...
  connector({
    ...
    unresolvedByDesign: true,
    trust: 'official-api',
    notes: 'Research metadata only; public no-key mode by default and personal key is quota-only. Not validation of technical claims. No company exposure inferred; authors kept minimal.',
  }),
  connector({
    id: 'crossref-works',
    label: 'Crossref',
  ...
  connector({
    ...
    unresolvedByDesign: true,
    trust: 'official-api',
    notes: 'Official UNECE/UNICC package by default; optional official CSV/package override. Location-code registry only — never live port activity, vessel traffic, congestion, or disruption.',
  }),
  connector({
    id: 'world-port-index',
  ...
  connector({
    ...
    access: 'public',
    cadence: 'periodic',
    sourceIdentity: 'USGS Mineral Resources (MRDS default, optional USMIN)',
    ...
    exposureSupport: 'curated-reference',
    trust: 'official-api',
    notes: 'MRDS public CSV by default as legacy reference (not updated since 2011); optional USMIN official export can be pinned. Reference only — never production/reserves/ownership/investment.',
  }),
  connector({
